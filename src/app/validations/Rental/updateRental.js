...
module.exports = async (req, res, next) => {
	try {
		const schemaRental = Joi.object({
			...
			cnpj: Joi.string().regex(/^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/).message('Your CNPJ should only contain characters accepted by the system!'),
			activities: Joi.string().min(5),
			address: Joi.array().min(1).items(
				{
					zipCode: Joi.string().regex(/^\d{5}-\d{3}$/).message('Your zipCode should only contain characters accepted by the system!'),
